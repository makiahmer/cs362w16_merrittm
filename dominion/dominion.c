...
/* TO_DO: ASSIGNMENT 1
3.	Pick 5 cards implemented in dominion.c. Refactor the code so
that these cards are implemented in their own functions, rather than
as part of the switch statement in cardEffect. You should call the
functions for these cards in the appropriate place in cardEffect.
Check in your changes, with appropriate svn commit messages.
Document your changes in a text file in your dominion source directory,
called “refactor.txt.” Your implementation of at least two of these
5 cards should be incorrect in some way. Do not document the bugs.
*/
int cardEffect_Adventurer(int drawntreasure, int currentPlayer, int cardDrawn, int temphand[], struct gameState *state) {
	int z = 0;	// counter for the hand
